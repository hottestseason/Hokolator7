...
public class Map {
	...
	public class Street implements Comparable<Street> {
		...
		private final Set<Pedestrian> neighborPedestrians = new HashSet<>();
		private Set<Street> neighborStreets;
		...
		public Set<Pedestrian> getNeighborPedestrians() {
			synchronized (neighborPedestrians) {
				return Collections.unmodifiableSet(neighborPedestrians);
			}
		}

		public Set<Street> getNeighborStreets() {
			if (neighborStreets == null) {
				neighborStreets = new HashSet<>(getSource().getStreets());
				neighborStreets.addAll(getTarget().getStreets());
			}
			return neighborStreets;
		}
		...
		public boolean remove(Pedestrian pedestrian) {
			for (Street neighborStreet : getNeighborStreets()) {
				synchronized (neighborStreet.neighborPedestrians) {
					neighborStreet.neighborPedestrians.remove(pedestrian);
				}
			}
			synchronized (pedestrians) {
		...
		private boolean add(Pedestrian pedestrian) {
			for (Street neighborStreet : getNeighborStreets()) {
				synchronized (neighborStreet.neighborPedestrians) {
					neighborStreet.neighborPedestrians.add(pedestrian);
				}
			}
			synchronized (pedestrians) {
